...
from utils import centralized_args_parser, binned_counter
...
def train_partition(
    ...
    binned_counter(combined_dataset, train_indices)
    print('Test dataset binned label counts')
    binned_counter(combined_dataset, test_indices)
    ...
    train_acc, test_acc, train_loss, test_loss, train_rmse, test_rmse = (
        eval_cnn(
            train_loader, test_loader, model, criterion, accuracy_tolerance
        )
    )
    partitions_path = (
    ...
    metadata = {
        ...
        'train root mean squared error': float(train_rmse),
        'test root mean squared error': float(test_rmse),
    ...
    save_cnn(model, metadata, f'dpcnn_oil_{model_id}')
