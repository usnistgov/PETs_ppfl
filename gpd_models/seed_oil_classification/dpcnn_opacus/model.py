...
class Net(nn.Module):
    def __init__(self, features: int):
        ...
        self.gn1 = nn.GroupNorm(num_groups=8, num_channels=8)
...
def train_cnn(
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        epoch_test_acc, _, _ = compute_accuracy(
    ...
    return train_acc, test_acc, losses, eps
...
def eval_cnn(model_id, train_loader, test_loader, model, criterion):
    ...
    print(
        f"Model {model_id} "
        f"| Accuracy on train set: {int(train_accuracy * 100)} %"
    )
    ...
    print(
        f"Model {model_id} "
        f"| Accuracy on test set: {int(test_accuracy * 100)} %"
    )
