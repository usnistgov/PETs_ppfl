...
class Net(nn.Module):
    def __init__(self, features: int):
        ...
        self.conv1 = nn.Conv1d(1, 12, kernel_size=14, padding=1)
        self.dropout1 = nn.Dropout1d(0.2)
        self.conv2 = nn.Conv1d(12, 10, kernel_size=10, padding=1)
...
def train_cnn(
    ...
    model.train()
    train_acc = []  # train acc per epoch
    test_acc = []  # test acc per epoch
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            labels = data[:, -1].long().to(device)  # Move labels to device
            optimizer.zero_grad()
